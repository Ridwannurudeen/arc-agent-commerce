...
- 118 Solidity tests + 57 Python SDK tests (9 skipped pending live-tx fixtures), CI green.
...
**Now (testnet, complete).** Two pipeline contracts deployed (clean redeploy 2026-05-08), evaluator-driven approval working, full audit→deploy flow exercised end-to-end with reputation recorded, frontend live, SDKs published (`arc-commerce-sdk` on PyPI and npm), 175 tests green.
...
- ~~Stand up an indexer for pipelines, stages, and reputation events so other Arc apps can query without reading contracts directly.~~ Done — Ponder indexer live at https://arc.gudman.xyz/indexer/graphql (GraphQL only, hardened: depth/alias/token caps + private metrics).
...
- **Integration partnerships** -- Build at least two reference integrations with other Arc projects (beyond the existing invoice-settlement example) to validate the primitive in real workflows.
